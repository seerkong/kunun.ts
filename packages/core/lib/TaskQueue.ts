...
export class TaskQueue {
    ...
    private runTask() {
        while (this.activeTaskNum < this.limit && this.queue.length > 0) {
            ...
            this.activeTaskNum++;
            // addTask returns void, so nobody can observe this promise:
            // swallow the rethrown error here to avoid unhandled rejections
            // (the failure is already logged by execute's catch).
            this.execute(task!).catch(() => undefined);
        }
    }
